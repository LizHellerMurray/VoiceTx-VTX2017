...
function createIndGoalTable(){
	...
	//finds completed indirect goals for subject and creates an array of their numbers to gray
	db.transaction(function (tx) {
		tx.executeSql('SELECT * FROM SESSION_INFO WHERE subj_num = ? AND goal_type = ?', [currSubjKey, 'Indirect'], function (tx, results) {	
	...
	//nested function to avoid asynch issues with text gray
		...
		db.transaction(function (tx) {
			tx.executeSql('SELECT * FROM INDGOAL_ID WHERE showIG = ?', [1], function (tx, results) {	
				...
				var currRow;
					for (var i = 0; i < len; i++){
						var doneTest = false;
						var iTest = results.rows.item(i).indID;
				...
				if(len == 0){
					document.getElementById("iTabMsg").innerHTML = "There are no indirect interevention goals saved in database, add your own below.";//should just go to add page?
				}else{
