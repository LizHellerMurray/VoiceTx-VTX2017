//some global variables established here
	...
	var currSessionID; 				//GUID for session, which tracks all goal info in session completed
	...
	var lastGoalID; 				//GUID identifier of last goal completed ***diff from currGoalID because identified w single dir goal text, same for whole "goal session", curr one chages within the session goal
	...
	var sessIDarray = [];			//array of GUIDs for all goal sessions in 
	...
	var indQuestArray = ["Identify the behavior", "Describe the effect of the behavior", "Define specific occurrences", "Plan for modifying behavior", "Other"]; 	//Vocal Hygiene Goal focus questions
	...
	//called from index.html after pass onload and deviceready setup functions
	function startDb() {

		db = window.openDatabase('vTxDBklg1', '1.0', 'VoiceTx', 2 * 1024 * 1024); //=3MB storage
		//DB name that went out to WI and WA (tables since updated): voiceTxDBmain1
		//for app upgrade, uncomment:
		//db.changeVersion("2.0", "2.1"); //change from, change to
		...
		//establishes subjects' documentation/performance tables
			...
			//tx.executeSql('DROP TABLE IF EXISTS PRESET_TRIALS'); //for testing purposes
			tx.executeSql('CREATE TABLE IF NOT EXISTS PRESET_TRIALS (preset_ID CHARACTER(36), subj_ref CHARACTER(36), goal_id VARCHAR(36), goal_text VARCHAR(255), methodsArray VARCHAR(255), toolsArray VARCHAR(255), uttLevel VARCHAR(255), delivLevel VARCHAR(255), PRIMARY KEY (preset_ID), FOREIGN KEY (subj_ref) REFERENCES SUBJ_INFO(subj_id))'); //can assume that goal type is always direct
...
/*
//function called whenever app is upgraded and launched AND the DB version is NOT the same
function onUpdate(db, oldVersion, newVersion){ //need to check if will stil call this
	var upgradeTo = oldVersion + 1;
	while (upgradeTo <= newVersion){ //while loop allows for updates to "catch up" if user misses one
		switch (upgradeTo){
			case 2:
				//upgrade to db version 2
				//db.executeSql("ADD SQL QUERY HERE TO ADD TABLE");
				db.transaction(function (tx) {
					//capture old db tables------- do i need to?
					//make changes to old db tables
					tx.executeSql('ALTER TABLE {tablename} ADD COLUMN {columnname} {type}', [], successHandler, errorHandler);
					alert("Updating!")
				});
				break;
			case 3:
				...
				break;
			//continue to edit with every new update
		}
...
function updateCurrInfo(showSubj, showInt, showGoal, showMeth, showTool, showDem, showUtt){
	...
	hideElems(["subjHeadBut", "intIndHeadBut", "intDirHeadBut", "indGoalHeadBut", "dirGoalHeadBut", "methHeadBut", "toolHeadBut", "demHeadBut", "uttHeadBut", "arrow1", "arrow2", "arrow3", "arrow4", "arrow5", "arrow6"]); //make all arrows hidden first, select which to show based on true parameters
